...
class ScrollState(object):
    ...
    def number_tx_success(self):
        number_tx_success = len([tx for tx in self.tx_list_external() if tx.get('txreceipt_status') == '1'])
        return number_tx_success

    def deposit(self):
        tx_list = self.tx_list_internal()
        volume = 0
        for tx in tx_list:
